...
import jacob.recess.openapi.api._
...
object CustomerStore extends IOApp {
  ...
  @SuppressWarnings(Array("org.wartremover.warts.Any", "org.wartremover.warts.Nothing", "org.wartremover.warts.ImplicitParameter"))
  def getServer(apiConf: ApiConfig, customerRepo: CustomerRepo[IO])(implicit actor: ActorSystem): Resource[IO, ServerBinding] = {
    Resource.make(IO.fromFuture(IO(Http().bindAndHandle(
      new CustomerRoutes(customerRepo).routes ~ new SwaggerAkka(CustomerApi.endpoints.toOpenAPI("Customer Api", "0.1.0").toYaml).routes,
      apiConf.host,
      apiConf.port,
      ))))(sb => IO.fromFuture(IO(sb.unbind())).map(_ => println("Server unbound from port.")))
  }

  def actorSystem(name: String): Resource[IO, ActorSystem] =
    Resource.make(IO(ActorSystem(name)))(as => IO(as.terminate()).map(_ => println("Actor system terminated")))
}
